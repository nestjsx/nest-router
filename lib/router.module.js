...
let RouterModule = RouterModule_1 = class RouterModule {
    ...
    static buildPathMap(routes) {
        routes.forEach(route => {
            route.children.path = validate_path_util_1.validatePath(validate_path_util_1.validatePath(route.path) + validate_path_util_1.validatePath(route.children.path));
        });
        routes.forEach(route => {
            Reflect.defineMetadata(MODULE_PATH, validate_path_util_1.validatePath(route.path), route.module);
            Reflect.defineMetadata(MODULE_PATH, validate_path_util_1.validatePath(route.children.path), route.children.module);
        });
    }
};
